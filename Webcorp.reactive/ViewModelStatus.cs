...
namespace Webcorp.reactive
{
    public class ViewModelStatus<T> : ReactiveObject, IViewModelStatus<T>, IDisposable where T : class
    {
        ...
        Dictionary<string, Action<ReactiveVMCollection<T>>> cache = new Dictionary<string, Action<ReactiveVMCollection<T>>>();
        IDisposable obs;
        ...
        public ReactiveVMCollection<T> ViewModel
        {
            get;  set;
        }

        public void Register(string v, Action<ReactiveVMCollection<T>> p)
        {
